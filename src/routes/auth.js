...
router.post('/register',
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existingUser) {
      ...
      throw new ConflictError('Unable to complete registration. Please try logging in.');
    }
    ...
    // Save refresh token
    user.addRefreshToken(refreshToken, REFRESH_TOKEN_EXPIRY, req.headers['user-agent'], req.ip);
    await user.save();
...
router.post('/logout', authenticate, asyncHandler(async (req, res) => {
  ...
  if (refreshToken && req.user) {
    try {
      ...
      // Log error but don't fail logout - client-side clearing is paramount
...
router.get('/google/callback',
  ...
  asyncHandler(async (req, res) => {
    ...
    // Save refresh token
    req.user.addRefreshToken(refreshToken, REFRESH_TOKEN_EXPIRY, req.headers['user-agent'], req.ip);
    await req.user.save();
    
    // SECURITY FIX: Use POST redirect instead of tokens in URL
    const frontendURL = process.env.FRONTEND_URL || 'http://localhost:5173';
    
    // Send HTML form that auto-submits via POST
    res.send(`
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        ...
        <noscript>
          <p>Redirecting... Please click the button if not redirected automatically.</p>
          <button onclick="document.getElementById('authForm').submit()">Continue</button>
        </noscript>
      </body>
      </html>
    `);
